...
<?php
	...
	if(($_SERVER["REQUEST_METHOD"]=="POST") && isset($_POST["delete"]))
	{
		...
		echo "if(question){ document.write(\"yes\"); } else { document.write(\"no\"); }";
		echo "</script>";
		//echo $check;
	}
...
<div class="wrapper col2">
  <div id="breadcrumb">
    <ul>
		...
		<li>&#187;</li>
		<li class="current"><a href="postNotice.php">All Notice</a></li>
    </ul>
  </div>
</div>
<!-- ####################################################################################################### -->
  ...
  <div id="container">
	...
	<div id="content">
      ...
      <div id="comments">
        ...
        <center><h2>Posted Notice</h2></center>
		...
		while($row = oci_fetch_array($result, OCI_BOTH)) 
		{
			...
			echo "<input type='hidden' name='item' value='" .$row[0]. "' />";
			echo '<p><ins><strong><a href="notice.php">'.$row[1].'</a></strong></ins></p>';
			echo '<p>Notice For: '.$row[4].'<p>';
			echo '<p>Notice Type: '.$row[3].'<p>';
			echo '<div class="submitdate"><a href="#">'.$row[5].'</a></div>';
			echo '<p>'.$row[2].'<p>';
			//echo '<input name="reply" type="submit" id="submit" value="Edit" /> &nbsp <button onclick="myFunction()">Delete</button>';
			//echo '<input name="edit" type="submit" id="submit" value="Edit" /> &nbsp <input name="delete" type="submit" value="Delete"/>';
