...
<?php
	...
	if(($_SERVER ["REQUEST_METHOD"]=="POST") && isset($_POST["submitUpdate"]))
	{
			...
			if(!empty($_POST["sId"]) && !empty($_POST["tId"]))
			{
				
				$connect = oci_connect("HR", "rat", "localhost/XE");
				
				$query = "UPDATE subject SET te_id = '".$tid."'WHERE subject_id='".$sid."'";
				
				$stid = oci_parse($connect, $query);
				oci_execute($stid);
				
				echo '<script language="javascript">';
				echo 'alert("Teacher Assign!!!")';
				echo '</script>';
			}
  ...
  <div id="breadcrumb">
    <ul>
      ...
      <li ><a href="adminLogin.php">Admin Home</a></li>
	   <li>&#187;</li>
      <li class="current"><a href="assTeacherSub.php">Assigning Teacher</a></li>
    </ul>
  </div>
</div>
<!-- ####################################################################################################### -->
<div class="wrapper col3">
  <div id="container">
	...
	<div id="content">
    <center><h1>View Class where teacher are not assign </h1></center>  
	<center>
	<form  method="post">
		<br/><br/>
	...
	<?php
			...
			$connect = oci_connect("HR", "rat", "localhost/XE");
			$query = "SELECT * FROM subject where te_id is null ORDER BY subject_id ASC";
		...
		//	echo '0.2';
				...
				echo '<td width="15" height="15"><center>'.$row[4].'</center></td>';
	...
	<br><br>
		<center><h1>Assign Teacher </h1></center>  
		<br>
		<table  border="1" >
		<tr>
			<td width="100" height="25">Subject ID</td>
			<td>
				<center><input type="text" name="sId" value="" style="width:250px;"/></center>
			</td>
		</tr>
		<tr>
			<td width="100" height="25">Teacher ID</td>
			<td>
				<center><input type="text" name="tId" value="" style="width:250px;"/></center>
			</td>
		</tr>
		
		</table>
		<center><input type="submit" name="submitUpdate" value=" Assign "/></center>
   </form>
