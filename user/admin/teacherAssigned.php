  ...
  <div id="breadcrumb">
    <ul>
      ...
      <li ><a href="adminLogin.php">Admin Home</a></li>
	   <li>&#187;</li>
      <li class="current"><a href="teacherAssigned.php">Assigned Class & Subject</a></li>
    </ul>
  </div>
</div>
<!-- ####################################################################################################### -->
<div class="wrapper col3">
  ...
  <center><h1>Search class</h1></center>  
	<center>
	<form  method="post">
		Teacher Id: <input type="text" name="tId" placeholder="Id Number"></input>
		<br/><br/>
	...
	&nbsp <input type="submit" name="submitSearch" value="Search"/>	
	</center>
	<br/><br/>
	...
	if(($_SERVER ["REQUEST_METHOD"]=="POST") && isset($_POST["submitSearch"]))
	{
		...
		//echo $serVal;
			...
			{
				echo '<script language="javascript">'; 
				echo 'alert("No subject is assigned to you according to search!!!")';
				echo '</script>';
		...
		elseif($serTy==2)
		{
			...
			{
				echo '<script language="javascript">'; 
				echo 'alert("Enter valid Subject ID!!!")';
				echo '</script>';
				
			}
			else
			{
				$query = "SELECT * FROM subject where (subject_id = '".$serVal."' and year = '".$seryr."' and te_id = '".$userId."')";
				$result = oci_parse($connect, $query);
		...
		elseif($serTy==3)
		{
			...
			{
				$query = "SELECT * FROM subject where (subject_name like '%$serVal%' and year = '".$seryr."' and te_id = '".$userId."')";
				$result = oci_parse($connect, $query);
		...
		elseif($serTy==4)
		{
			...
			{
				$query = "SELECT * FROM subject where (cls = ".$serVal." and year = '".$seryr."' and te_id = '".$userId."')";
				$result = oci_parse($connect, $query);
